...
function CheckIn() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Workout generation failed. Please try again.";
      toast.error(message);
    } finally {
      setGenerating(false);
    }
      ...
      <div className="mx-auto max-w-2xl">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-muted-foreground">
            Tune today's {schedule.label.toLowerCase()} session to your body.
          </p>
        </div>
    ...
    <div className="rounded-2xl border border-border bg-card p-5">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div
            className={`flex h-10 w-10 items-center justify-center rounded-lg bg-secondary ${accentClass}`}
          >
            {icon}
          </div>
          <div>
        ...
        <div className={`font-display text-3xl font-bold ${accentClass}`}>
          {value}
      ...
      <input
        ...
        style={{
          accentColor: color === "primary" ? "oklch(0.78 0.18 155)" : "oklch(0.82 0.17 80)",
        }}
      />
